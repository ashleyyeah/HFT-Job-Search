...
def extractSkills():
    ...
    skill_found = list()
    id = 0
    with open('company_role_skill/company_role_skill.csv','w', newline="\n")  as skill_file:
                ...
                if row[0] != "COMPANY_ROLES_ID":
                    ...
                    for skill in skill_dict:
                        if(role_name.find(skill.upper()) != -1):
                            ...
                            with open('company_role_skill/company_role_skill.csv','a', newline="\n")  as skill_file:
                                writer = csv.writer(skill_file)
                                to_write = [id, row[0],skill_dict[skill]]
                                writer.writerow(to_write)
                                id=id+1
